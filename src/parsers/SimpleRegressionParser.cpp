#include "SimpleRegressionParser.h"

SimpleRegressionParser::SimpleRegressionParser() {

}

string SimpleRegressionParser::nameParser() {
    return "SimpleRegressionParser";
}

void SimpleRegressionParser::Optimize(Subject* s) {

}

void SimpleRegressionParser::setDataSet(double ** x,int tam) {
    tamDataset = tam;
    dataset = x;
}

void SimpleRegressionParser::printResult(Subject * s) {}

double SimpleRegressionParser::Evaluate(Subject* s) {
    //return  1;
    double r = 0;
    double fit = 0 ;
    ...
    for(int arvore = 0; arvore < conf->numTree; arvore++) {
        ...
        for( int j = 0; j <tamDataset; j++) { // para todos os dados do conjunto de treinamento
            ...
            // Para regressao
            fit += pow(r - dataset[j][Database::getInstance().variables + arvore], 2);
...
double SimpleRegressionParser::Operate(int opType, int opValue, double a, double b, double c) {
    double r;
...
double SimpleRegressionParser::AuxEvaluate(Subject* s, int model, double* dat) {
